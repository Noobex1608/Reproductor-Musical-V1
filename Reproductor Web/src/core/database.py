...
class DatabaseManager:
    ...
    async def scan_and_add_file(self, file_path: str) -> bool:
        """Escanea un archivo de audio y lo agrega a la base de datos"""
        try:
            from pathlib import Path
            import mutagen
            ...
            logger.debug(f"🔍 Escaneando archivo: {file_path_obj.name}")
            
            # Verificar si ya existe en la base de datos
            file_hash = self._get_file_hash(file_path)
            existing = await self.get_song_by_hash(file_hash)
            if existing:
                logger.debug(f"⚠️ Archivo ya existe en DB: {file_path_obj.name}")
                return True  # Ya existe, no es un error
            
            # Extraer metadatos usando mutagen
            try:
                audio_file = mutagen.File(file_path)
                if audio_file is None:
                    logger.warning(f"❌ No se pudo abrir archivo de audio: {file_path_obj.name}")
                    return False
                
                # Extraer información básica
                song_data = {
                    'file_path': str(file_path_obj.absolute()),
                    'title': str(audio_file.get('TIT2', [file_path_obj.stem])[0]) if audio_file.get('TIT2') else file_path_obj.stem,
                    ...
                    'year': None,
                    'duration': getattr(audio_file.info, 'length', 0.0) if hasattr(audio_file, 'info') else 0.0,
                    'bitrate': getattr(audio_file.info, 'bitrate', None) if hasattr(audio_file, 'info') else None,
                    'sample_rate': getattr(audio_file.info, 'sample_rate', None) if hasattr(audio_file, 'info') else None,
                ...
                if audio_file.get('TDRC'):
                    try:
                        ...
                        song_data['year'] = int(year_str[:4]) if len(year_str) >= 4 else None
                    except (ValueError, IndexError):
                        pass
                
                # Agregar a la base de datos
                logger.debug(f"💾 Agregando a DB: {song_data['title']} - {song_data['artist']}")
                song_id = await self.add_song(song_data)
                success = song_id is not None
                
                if success:
                    logger.debug(f"✅ Archivo agregado exitosamente con ID {song_id}: {file_path_obj.name}")
                else:
                    logger.warning(f"❌ No se pudo agregar a DB: {file_path_obj.name}")
                
                return success
                
            except Exception as metadata_error:
                logger.error(f"❌ Error extrayendo metadatos de {file_path_obj.name}: {metadata_error}")
                # Fallback: agregar con información mínima
                song_data = {
                    'file_path': str(file_path_obj.absolute()),
                ...
                logger.debug(f"🔄 Usando fallback para: {song_data['title']}")
                song_id = await self.add_song(song_data)
                success = song_id is not None
                
                if success:
                    logger.debug(f"✅ Archivo agregado con fallback (ID {song_id}): {file_path_obj.name}")
                
                return success
                
        except Exception as e:
            logger.error(f"❌ Error escaneando archivo {Path(file_path).name}: {e}")
            return False
    ...
    async def cleanup(self):
        ...
        try:
            # Limpiar biblioteca musical antes de cerrar la conexión
            logger.info("🧹 Limpiando biblioteca musical antes del cierre...")
            await self.clear_music_library()
    ...
    async def clear_music_library(self):
        """Limpia completamente la biblioteca musical manteniendo configuraciones"""
        try:
            cursor = self.connection.cursor()
            
            # Limpiar todas las tablas relacionadas con música
            ...
            # Resetear secuencias de ID si es necesario
            cursor.execute("DELETE FROM sqlite_sequence WHERE name IN ('songs', 'playlists', 'playlist_songs', 'play_history', 'favorites')")
            
            self.connection.commit()
            logger.info("🧹 Biblioteca musical limpiada completamente")
            
        except Exception as e:
            logger.error(f"Error limpiando biblioteca musical: {e}")
            self.connection.rollback()
    
    def clear_music_library_sync(self):
        """Versión síncrona para limpiar la biblioteca musical"""
        try:
            cursor = self.connection.cursor()
            ...
            self.connection.commit()
            logger.info("🧹 Biblioteca musical limpiada completamente al cerrar")
    ...
    def get_library_status(self):
        """Verifica el estado de la biblioteca musical"""
        try:
            cursor = self.connection.cursor()
            cursor.execute("SELECT COUNT(*) FROM songs")
            ...
            return {
                ...
                'is_empty': song_count == 0 and playlist_count == 0
            }
        except Exception as e:
